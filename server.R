...
function(input, output){
  ...
  mymap <- get_map(location = "New York", maptype = "terrain", color="bw")
  ...
  output$plot1 <- renderPlot({
    ...
    ggmap(mymap)  + geom_point( data=dat, aes(x=lon, y=lat), size=10
                                , col="lightgoldenrod2", alpha=0.7
    ) +
      geom_point( data=dat, aes(x=lon, y=lat), size=7, col="light blue", density = 50
      ) 
